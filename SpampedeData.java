...
class SpampedeData {
	...
	public void moveSnakeForward(BoardCell cell) {
		...
		this.moveSnakeHead(cell);
	}

	public void moveSnakeHead(BoardCell cell) {
	...
	public BoardCell getNextCellInDir() {
		...
		} else if (this.currentMode == SnakeMode.GOING_NORTH) {
	...
	public void reverseSnake() {
		System.out.println("original" + this.snakeCells);
		// Step 1: unlabel the head
		this.getSnakeHead().becomeBody();
		// Step 2: reverse the body parts
		LinkedList<BoardCell> rSnakeCells = new LinkedList<BoardCell>();
		int size = this.snakeCells.size();
		for (int i = 0; i < size; i++) {
			rSnakeCells.add(this.snakeCells.getLast());
			this.snakeCells.removeLast();
		}

		snakeCells = rSnakeCells;
		...
		if (this.getNorthNeighbor(neck) == head) {
			this.currentMode = SnakeMode.GOING_NORTH;
		} else if (this.getEastNeighbor(neck) == head) {
			this.currentMode = SnakeMode.GOING_EAST;
		} else if (this.getWestNeighbor(neck) == head) {
			this.currentMode = SnakeMode.GOING_WEST;
		} else {
			this.currentMode = SnakeMode.GOING_SOUTH;
		}
