...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		// Draw the board
		int verticalOffset = (this.height - (Preferences.GAMEBOARDHEIGHT))/2;
		int horizontalOffset = (this.width - (Preferences.NUM_CELLS_WIDE * Preferences.CELL_SIZE))/2;
		for (int r = 0; r < this.theData.getNumRows(); r++) {
			for (int c = 0; c < this.theData.getNumColumns(); c++) {
				this.drawSquare(c*10+horizontalOffset, r*10+verticalOffset, this.theData.getCellColor(r, c));
			}
		}
