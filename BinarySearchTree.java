...
public class BinarySearchTree<KeyType extends Comparable<KeyType>, ValueType> implements Map<KeyType, ValueType> {
	...
	private KeyType getMinKey(BSTNode root) {
		if (root == null) {
			throw new IllegalArgumentException("root arguement is null"); // if the root is null
		} else if (root.left != null) {
			return getMinKey(root.left); // recursively call until the next left root equals null
		}
		return root.key;
	}
	...
	private BSTNode remove(KeyType key, BSTNode root) {
		if (root == null) { // reaches end of tree without finding key
			return root;
		} else if (inOrderKeys(root.key,key)) { // recurse the right side of the tree
			root.right = remove(key, root.right);
		} else if (inOrderKeys(key, root.key)) {
			root.left = remove(key, root.left); // recurse the left side of the tree
		} else {
			if (root.right == null && root.left == null) { // node is a leaf
				this.size--;
				return null;
			} else if (root.left == null) { // node has a right child
				this.size--;
				return root.right;
			} else if (root.right == null) { // node has a left child
				this.size--; 
				return root.left;
			} else { // node has two children
				KeyType smallestKey = getMinKey(root.right);
				root.value = get(smallestKey,root.right);
	...
	private void addKeysToArrayList(ArrayList<KeyType> keys, BSTNode root) {
		if (root != null) {
			addKeysToArrayList(keys, root.left); // recurses with the left root of the tree
			keys.add(root.key); // adds the head to the ArrayList
			addKeysToArrayList(keys, root.right); // recurses with the right root of the tree
		}
	}
